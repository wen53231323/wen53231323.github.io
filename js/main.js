document.addEventListener('DOMContentLoaded', function() {
	...
	const addHighlightTool = function() {
		...
		const $figureHighlight = highLight.plugin === 'highlighjs' ? document.querySelectorAll(
			'figure.highlight') : document.querySelectorAll('pre[class*="language-"]')
		...
		if (isHighlightLang) {
			if (isPrismjs) {
				$figureHighlight.forEach(function(item) {
					const langName = item.getAttribute('data-language') ? item.getAttribute(
						'data-language') : 'Code'
	...
	const runJustifiedGallery = function(ele) {
		...
		const lazyloadFn = (i, arr, limit) => {
			...
			if (arrLength > loadItem) i.insertAdjacentHTML('beforeend', htmlStr(arr.splice(0,
				loadItem)))
			else {
		...
		const runJustifiedGallery = (item, arr) => {
			if (!item.classList.contains('lazyload')) item.innerHTML = htmlStr(arr)
				...
				const clickBtnFn = () => {
					...
					fjGallery(item, 'appendImages', item.querySelectorAll(
						`.fj-gallery-item:nth-last-child(-n+${lastItemLength})`))
					btf.loadLightbox(item.querySelectorAll('img'))
					lastItemLength < limit && item.nextElementSibling.removeEventListener('click',
						clickBtnFn)
				}
	...
	const scrollFnToDo = function() {
		...
		if (isToc) {
			...
			$cardToc.addEventListener('click', e => {
				...
				btf.scrollToDest(btf.getEleTop(document.getElementById(decodeURI($target
					.getAttribute('href')).replace('#', ''))), 300)
	...
	const rightSideFn = {
		...
		runMobileToc: () => {
			if (window.getComputedStyle(document.getElementById('card-toc')).getPropertyValue(
				'opacity') === '0') window.mobileToc.open()
	...
	const switchComments = function() {
		...
		$switchBtn && $switchBtn.addEventListener('click', function() {
			...
			document.querySelectorAll('#post-comment > .comment-wrap > div').forEach(function(
			item) {
	...
	const unRefreshFn = function() {
		window.addEventListener('resize', () => {
			adjustMenu(false)
			btf.isHidden(document.getElementById('toggle-menu')) && mobileSidebarOpen && sidebarFn
				.close()
		})
	...
	window.refreshFn = function() {
		...
		if (GLOBAL_CONFIG_SITE.isPost) {
			...
			GLOBAL_CONFIG.relativeDate.homepage && relativeDate(document.querySelectorAll(
				'#recent-posts time'))
...
document.addEventListener("visibilitychange", function() {
	document.hidden ? (document.title = "☹页面不见了~", clearTimeout(st)) : (document.title = "☺欢迎回来！", st =
		setTimeout(function() {
